...
export default function AppTime() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setLoading(false); // Desativar o loader após o carregamento
    }
  };
  ...
  return (
    <Box sx={{ padding: '20px' }}>
      <Container fixed sx={{ display: 'flex', flexDirection: 'column', mt: '40px' }}>
        <h2 style={{ textAlign: 'center', color: 'yellow', lineHeight: '30px', letterSpacing: '2px', textShadow: '4px -1px 1px black' }}>
        ...
        />
        <Button variant="contained" color="primary" onClick={handleSearch} sx={{ marginTop: '20px' }}>
          Search
        </Button>
        {loading ? (
